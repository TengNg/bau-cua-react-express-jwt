...
export default function PlayerBetSettings({ isOpen, handleClose, currentPlayerId, getCurrentPlayer, handleSetPlayerBetInfo, betRange }) {
    ...
    useEffect(() => {
        if (currentPlayerId) setItems(getCurrentPlayer().betInfo);
    }, [currentPlayerId]);
    ...
    const handleBetMoneyChanged = (e, id) => {
        if (e.target !== e.currentTarget) return;
        const value = Math.max(minBet, Math.min(maxBet, +e.target.value));
        setItems(currItems => [...currItems].map(item => item.id === id ? { ...item, betMoney: value } : item));
    };
            ...
            >
                ...
                <div className='section--style grid grid-cols-3 grid-rows-2 p-4 gap-4 items-center justify-items-center' >
                    {items.map((item, index) => {
                        return (<div
                            key={index}
                            className={`w-[150px] h-[150px] border-black border-[4px] font-bold flex--center flex-col gap-3 cursor-pointer ${item.selected ? 'bg-gray-400' : 'bg-gray-100'}`}
